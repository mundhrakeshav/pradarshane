...
const TransferableOwnableTokenMintPage = () => {
  ...
  const [
    transferableOwnableContract,
    setTransferableOwnableContract,
  ] = useState();
  ...
  useEffect(() => {
    const initialise = async () => {
      const _contract = await getTransferableOwnableNFTContract(address);
      ...
      setTransferableOwnableContract(_contract);
      setName(_name);
      setSymbol(_symbol);
            ...
            <Button
              ...
              onClick={async () => {
                web3.eth.getAccounts(async (error, accounts) => {
                  ...
                  const resp = await transferableOwnableContract.methods
                    .mintToken(mintAddress, uniqueTokenName, ipfsImageHash)
                    .send({ from: accounts[0] });
                });
                axios
                  .post(
                    "http://localhost:5000/addTransferableOwnableContracts",
                    {
                      address,
                      mintAddress,
                    }
...
export default TransferableOwnableTokenMintPage;
